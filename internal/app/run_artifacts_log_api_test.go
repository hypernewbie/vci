...
// Run-data read surfaces: artifact collection (CollectArtifacts), the
// coordinator-side artifact read API (ListArtifacts / GetArtifact), and
// the coordinator-side log read API (ReadLog). The client ssh proxy
// tests previously living here are owned by the CLI command tests
// (internal/cli/artifacts_logs_test.go), which prove the same
// raw-data and public-error contracts end to end; only
// coordinator-side data behavior is pinned here.
...
func setupArtifactsRun(t *testing.T) (model.Layout, model.RunID) {
	t.Helper()
	l := model.Layout{Root: t.TempDir()}
...
func setupLogsRun(t *testing.T) (model.Layout, model.RunID) {
	t.Helper()
	l := model.Layout{Root: t.TempDir()}
	if err := l.Ensure(); err != nil {
		t.Fatal(err)
