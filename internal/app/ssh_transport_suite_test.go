...
// TestStagedTreeAtPublicCommandBoundary replaces the final remote
// `vci build .` with a probe asserting the staged tree:
//
//   - .git/HEAD, objects/, and refs/ are present (git rev-parse works);
//   - tracked, modified, and untracked files carry expected bytes;
//   - executable mode and symlink target survive;
//   - .git/config and ignored files were never staged.
//
// The probe returns the standard Vci envelope so the client sees a
...
func TestStagedTreeAtPublicCommandBoundary(t *testing.T) {
	...
	// Runs in place of `vci build .` from inside the staged project
	// dir. Every assertion is a hard failure: the missing fact lands on
	// stderr and the non-zero exit surfaces as an SSH failure.
	probe := `#!/bin/sh
set -eu
...
`
	...
	// Written after the commit so it uses the fixture identity; the
	// sentinel must not reach the staged tree.
	mustWriteFile(t, filepath.Join(sourceDir, ".git", "config"), "[user]\n  name = PrivateSentinelUser\n")

	// Modified tracked file: working-tree bytes must arrive.
	mustWriteFile(t, filepath.Join(sourceDir, "modified.txt"), "modified content")
